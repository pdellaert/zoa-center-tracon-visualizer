...
4. **Render**: `ProcedurePoints` consumes `displayedProcedures`. For each procedure, `buildProcedureGeometry` walks all sequences applying per-kind rules:
   - SID: skip `HeadingToAltitude`/`CourseToAltitude` (no coords); seed `lineCoords` at `sequence.runwayOrigin` if set; `HeadingToManual`/`CourseToManual`/`FromFixToManual`/`ManualTermination` render as a chevron arrowhead along `point.course` (also applies to STAR/APP).
...
- Layer ids are meaningful — `BaseMapColorSync` matches by prefix/suffix patterns. When adding new layers, check that regex.
