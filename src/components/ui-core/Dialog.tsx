...
const DialogOverlay = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, DialogOverlayProps>) => {
  ...
  return (
    <DialogPrimitive.Overlay
      class={cn(
        'fixed inset-0 z-50 bg-slate-300/80 backdrop-blur-sm data-expanded:animate-in data-closed:animate-out data-closed:fade-out-0 data-expanded:fade-in-0',
        props.class,
      )}
      {...rest}
    />
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        class={cn(
          'fixed left-1/2 top-1/2 z-50 grid w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 bg-gray-100 p-6 drop-shadow-lg duration-200 data-expanded:animate-in data-closed:animate-out data-closed:fade-out-0 data-expanded:fade-in-0 data-closed:zoom-out-95 data-expanded:zoom-in-95 data-closed:slide-out-to-left-1/2 data-closed:slide-out-to-top-48% data-expanded:slide-in-from-left-1/2 data-expanded:slide-in-from-top-48% sm:rounded-lg',
          props.class,
        )}
        {...rest}
      >
